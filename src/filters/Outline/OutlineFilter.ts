...
export class OutlineFilter extends Phaser.Filters.Controller {
    ...
    constructor(
        ...
        const renderer = camera.scene.sys.renderer as Phaser.Renderer.WebGL.WebGLRenderer;
        const renderNodes = renderer.renderNodes as any;

        if (!renderNodes.hasNode('FilterOutline')) {
            renderNodes.addNode('FilterOutline', new OutlineFilterRenderNode(renderNodes));
        }
